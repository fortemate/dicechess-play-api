...
- **Stalled opponents**: A player whose opponent has stopped moving should wait for their clock to run out (`Timeout`) rather than resigning. An opponent who has actually dropped the connection is forfeited by the server after the disconnect grace, which ends the game as `Resign`.
