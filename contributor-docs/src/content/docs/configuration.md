...
| `RATING_POLICY` | Optional, `legacy` (default) or `matrix` (#146). Which eligibility matrix NEW games are classified under: `legacy` rates any two registered, non-anonymous participants on a bounded control; `matrix` is the bot-rating-boundaries ADR — two accounts compete, two bots compete only when the ladder scheduler paired them, a human against a bot is a `training` game (`rated: false`, `ratingDomain: "training"`, no canonical rating moves), everything else casual. Every `game_results` row records the policy version that classified it; flipping the value never reclassifies existing rows. |
...
| `PLAY_RATED_FOR_HUMANS` | Deprecated since #279; ignored if set. Rated play is chosen by the player at game/seek creation. Boot logs a warning if present. |
...
| `PLAY_DRAW_REOFFER_TURNS` | Optional, default `0` (non-positive value means the right never returns on its own). Controls after how many of a seat's own completed turns the right to offer a draw returns without an opponent's offer. Under the default, draw offers follow a strict passing right (the right transfers to the opponent upon offering and only returns if the opponent offers back). Setting a positive integer (e.g. `5`) enables automatic return after N turns, loosening the anti-spam rule and producing draw offer cooldowns with `availableAfterTurns`. |
