...
import dicechess.play.store.{AdminBotStore, BotCatalogState, BotRating, UserAccount}
...
/** One registered bot in the administrator's inventory (#313). `owned` says only whether some account owns it — the
  * operator can badge self-service candidates without learning an owner's identity, which is irrelevant to the admin's
  * deliberately ownership-free authority.
  *
...
/** `GET /admin/bots`' full, intentionally unpaginated registry (#313). */
...
/** The administrator's door to any registered bot (#273) — the third door after the bot's own Bearer token and the
  ...
  * `RotatedToken`, `ConfirmRotation`) so a client sees one vocabulary. A signed-in non-admin gets **403**, not 404 —
  * the owner surface's honesty argument: the public `GET /bots/{team}/{name}` already reveals existence, and this
  * repository is public, so hiding the surface would protect nothing. 404 stays "no such registered bot".
  *
...
object AdminBotRoutes:
  ...
  def apply(session: AuthSession, auth: BotAuth, admins: Set[String], store: AdminBotStore): HttpRoutes[IO] =
    HttpRoutes.of[IO]:
              ...
              AdminBots(
                bots.map: bot =>
                  AdminBot(
                    ...
                    bot.rating,
                    bot.rd,
                    // Computed here, not in the store, exactly as `CatalogRoutes` does it: the threshold is a wire
                    // concern, and the store stays a plain projection of the row.
